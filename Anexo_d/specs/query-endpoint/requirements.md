...
**Status:** Aceito
**Contexto:** Exercício 2.1 — Cenário-Âncora 2 (Trilha AI First DGS)
...
Derivado dos bounded contexts da seção 1:
...
> Este módulo é constrangido pelas seguintes decisões do Cenário 1. Qualquer mudança de arquitetura deve passar pela revisão das ADRs correspondentes.
...
|---|---|---|---|
| 1 | *"Os outcomes da v1 eram features técnicas, não resultados. 'Endpoint retorna JSON com source_document' não diz nada para o atendente."* | Outcomes orientados a componente, não ao usuário final | Reescrevi todos os 8 outcomes em perspectiva do atendente (ex: "O atendente recebe resposta em ≤ 30s") |
| 2 | *"Falta referência ao ADR-0002 (context budget). O limite de 3.000 tokens impacta diretamente o número de chunks enviados ao LLM e, portanto, a latência e a qualidade das respostas."* | Requisito não derivado das decisões do Cenário 1 | Adicionei FR-07 e NFR-01 com referência explícita ao ADR-0002; incluí na seção 9 |
| 3 | *"Os critérios de verificação da v1 eram ambíguos: 'Resposta deve mencionar o documento correto'. Isso não é binário — o QA não sabe como verificar."* | Critérios não testáveis (nível "resposta deve ser boa") | Reescrevi todos os 12 VCs com input concreto, condição de PASS explícita e arquivo de teste associado |
| 4 | *"A v1 não tratava o conflito PROC-042 v1 vs v2. Esse é o caso de borda mais crítico do sistema."* | Omissão do cenário de conflito de versões — maior fonte de risco financeiro | Adicionei FR-02, VC-03, VC-04 e a linha ADR-0003 na seção 9 |
| 5 | *"O scope boundary estava genérico. 'Não cobre gestão documental' sem dizer por quê e o que fazer confunde o desenvolvedor e o LLM."* | Scope genérico sem justificativa nem ação alternativa | Reescrevi seção 5.2 com tabela incluindo justificativa e ação de escalonamento |

---

## 11. Mockup — Interface do Query Endpoint (Painel Web do Atendente)

> Representação do componente de consulta do assistente, coerente com os requirements acima. Exibe: fonte, confiança e feedback — conforme OUT-02, OUT-07 e VC-06/VC-07.
> *Mockup textual produzido como referência de design antes da implementação em `src/web/`. Substitui mockup Claude Design para fins de documentação.*
...
│                                                                      │
├──────────────────────────────────────────────────────────────────────┤
│  ── Segundo exemplo: confiança baixa ──────────────────────────────  │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐     │
│  │  ⚠️  ATENÇÃO: Confiança baixa (0.48)                        │     │
│  │  As informações disponíveis podem não cobrir este caso.     │     │
...
│  Para fretes padrão, consulte a tabela mensal de fretes.             │
│                                                                      │
│  📄 Fonte: PROC-042v2 §1 (definição de frete especial)               │
│                                                                      │
│  Esta resposta foi útil?   [👍 Sim]  [👎 Não]  [⚑ Reportar erro]   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘

Legenda dos indicadores de confiança:
  ≥ 0.8  → Alta    (verde) — resposta direta ao cliente
  0.6–0.79 → Média (amarelo) — verificar antes de repassar
  < 0.6  → Baixa  (vermelho) — confirmar com supervisor obrigatório
```
...
|---|---|---|---|
| OQ-01 | PROC-043 (frete de carga perigosa) está em revisão pelo Compliance. Quando estiver disponível, FR-04 precisará de atualização. | Médio — cálculo de frete para carga perigosa > 500kg não coberto | Aguardando Compliance |
| OQ-02 | O FAQ-Atendimento (Chunk FAQ-03) sugere que Gestão de Riscos "já autorizou exceções" para devolução de carga perigosa. Esse comportamento informal conflita com a POL-001 §3.2. O assistente deve mencionar a possibilidade de exceção? | Alto — risco de criar expectativa não-garantida para o cliente | Escalar para PO e Diretoria de Operações |
| OQ-03 | O sistema de tracking não está no escopo de ingestão RAG. Consultas como "onde está minha carga?" serão frequentes mas não respondíveis. Criar resposta padrão de redirecionamento? | Médio — experiência do atendente degradada sem fallback claro | Definir com Tech Lead e UX |
...
|---|---|---|
| **D1 — Domínio Conceitual** | **3** | O documento demonstra compreensão de: (a) SDD como formato de spec orientado a outcomes, não a features técnicas — diferença aplicada concretamente na reescrita da seção 4; (b) bounded contexts como divisão por domínio de negócio, não por camada técnica (seção 1); (c) linguagem ubíqua como conjunto de termos que um LLM confundiria sem definição explícita (seção 2, 15 termos); (d) verification criteria como requisito binário testável por QA (seção 8, 12 VCs com input/condição/arquivo); (e) AGENTS.md como destino de consumo — seção 9 referencia ADRs que serão transcritos no Ex. 2.3. |
| **D2 — Uso de Ferramentas** | **3** | Claude foi usado para gerar a v1 e incorporar 5 feedbacks do Tech Lead na v2. A iteração está documentada formalmente na seção 10 com: problema da v1, diagnóstico e ação concreta na v2. Diferença estrutural verificável: outcomes reescritos (de features para resultados), VCs tornados binários, FR-02/VC-03/VC-04 adicionados para tratar conflito PROC-042, scope boundaries reescritos com justificativa e escalonamento. |
| **D3 — Qualidade do Entregável** | **3** | Artefato completo: bounded contexts mapeados com diagrama ASCII (seção 1), glossário de 15 termos com risco de confusão identificado (seção 2), 8 outcomes orientados a resultado (seção 4), scope boundaries com justificativa e ação (seção 5), 7 FRs e 5 NFRs (seções 6–7), 12 VCs binários com arquivo de teste (seção 8), mockup coerente com requirements exibindo fonte + confiança + feedback (seção 11), open questions rastreadas (seção 12). |
| **D4 — Pensamento Crítico** | **3** | Julgamentos autônomos demonstrados: (a) OQ-02 identifica conflito entre comportamento informal do FAQ-03 e a POL-001 §3.2 — ambiguidade escalada para PO, não resolvida arbitrariamente; (b) OQ-03 antecipa a lacuna de tracking em tempo real como fonte de frustração do atendente — caso não documentado no enunciado; (c) o mockup diferencia três níveis de confiança com comportamento distinto para cada um, refletindo que confiança moderada (0.6–0.79) não deve bloquear o atendente, apenas alertar. |
| **D5 — Aplicabilidade ao Projeto** | **3** | Totalmente conectado à NovaTech: glossário extraído do Anexo A (termos reais dos docs), bounded contexts derivados dos 4 documentos fonte (POL-001, PROC-042, SLA-2024, FAQ), VCs testam casos reais (VC-02 tier Platinum, VC-03/04 conflito de multiplicadores, VC-05 carga perigosa). Referência explícita a ADR-0001, ADR-0002 e ADR-0003 na seção 9, com impacto concreto em FRs e NFRs. |
